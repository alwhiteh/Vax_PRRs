...
# colnames(mouse_genes) <- c("gene")
# filt_conv_table <- conv_table[mouse_genes$gene 
#                               %in% conv_table$mmusculus_homolog_associated_gene_name,]
...
# Load mouse sample from matrices 
data <- fread("/Users/alexwhitehead/sc_RNA_raw_data/Lung/Mouse/EGAS00001004333/mouse_droplet_TMS_UMIs.csv")
meta <- read.csv("/Users/alexwhitehead/sc_RNA_raw_data/Lung/Mouse/EGAS00001004333/mouse_droplet_TMS_metadata.csv")
data <- as.matrix(data)
rownames(data) <- data[,1]
data <- data[-1]
kassay <- CreateAssayObject(counts = data)
# the metadata from 10x has an extra X at the beginning of the colnames (7385 - 24618)
test <- meta$X[7385:24618]
test2 <- paste("X",test, sep = "")
meta$X[7385:24618] <- test2

help <- setdiff(meta$X, colnames(data))
# insert function to convert gene names
m_lung_kras <- CreateSeuratObject(data, meta.data = test, assay = "RNA", min.cells = 3, min.features = 200, project = "m_lung_kras")
...
After saving the seurat objects, they can be merged using the CHTC scripts. Alternatively, we need to map the mouse genes to human orthologs in order to be able to combine the datasets. This requires changing the feature names prior to generating the Seurat Object.
...
# Try a test case, then generate a function to apply to multiple objects
# load test case 
pbmc_small = SeuratObject::pbmc_small
geneset <- rownames(pbmc_small)
...
### Now lets try a real test case using the mouse krasnow data
dummy <-readRDS("m_lung_rare_f.RDS")
mouse_genes <- as.data.frame(dummy@assays$RNA@data@Dimnames[[1]])
colnames(mouse_genes) <- c("gene")

# Let's make sure our conversion table has all the genes we need:
# Raredon: 24k genes, sampath: 17k genes, krasnow: 23.4k
# This points out a limitation, not all mouse genes have orthologs with human genes
# We will have to implement an %in% operator to make this easier
filt_conv_table <- conv_table[mouse_genes$gene 
                              %in% conv_table$mmusculus_homolog_associated_gene_name,]
merg_conv_table <- mouse_genes %>%
  left_join(filt_conv_table, by = c("gene" = "mmusculus_homolog_associated_gene_name"))
# This returns the total mouse count data and homologue genes are replaced when possible

## We will now add the mouse genes into the same column as the human orthos
# make empty values into NAs
 merg_conv_table$external_gene_name[merg_conv_table$external_gene_name==""] <- NA
# replace NAs with values from "gene" column
merg_conv_table %>% 
    mutate(external_gene_name = coalesce(external_gene_name,gene)) -> merg_conv_table
# Some values are duplicated due to transcripts 1-2 basepairs longer
merg_conv_table <- merg_conv_table[!duplicated(merg_conv_table$gene),]
...
RPCA without the stripp samples seems to have best integrated the cells across datasets. A few exceptions are: cluster 1 is specific to Okuda (the unidentified cells from their manuscript), and and cluster 10 is the nasal goblet cells that are only found in the Teich nasal data. Moving forward, we will use the h_rpca_ns data for the human lung + nose. This is congruent with the muring approach, which also uses rPCA.
