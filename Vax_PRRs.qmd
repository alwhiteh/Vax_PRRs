...
#| echo: FALSE
mlung.markers <- read.csv("/Users/alexwhitehead/Documents/Postdoc/Bl_Eng2_Priming/Vax_PRRs/mouse/mlung_markers.csv")
...
counts <- GetAssayData(object = mlung.combined, slot = "data")
...
meta <- meta[,c(1,2,3,4,5,15,17,18,28)]
hu_genes <- mouse_to_human(counts@Dimnames[[1]], conv_table)
counts@Dimnames[[1]] <- hu_genes$human_ortho
new_obj <- CreateAssayObject(counts)
mlungh <- CreateSeuratObject(new_obj, meta.data = meta, assay = "integrated", project = "m_lung_h")
saveRDS(mlungh, "mouse_as_human/m_lung_h_ripped.RDS")


# # # Do normal preprocessing and make sure it works - we will use SeuratV3 workflow
# mlungh <- readRDS("mouse_as_human/m_lung_h_ripped.RDS")
# mlungh <- SCTransform(mlungh, method = 'glmGamPoi', verbose = FALSE)
# #DefaultAssay(mlungh) <- 'SCT'
# # mlungh <- NormalizeData(mlungh, verbose = FALSE)
# # mlungh <- FindVariableFeatures(mlungh, selection.method = "vst", nfeatures = 2000)
# # all.genes <- rownames(mlungh)
# # mlungh <- ScaleData(mlungh, features = all.genes)
# mlungh <- RunPCA(mlungh, features = VariableFeatures(object = mlungh), npcs = 30)
# mlungh <- RunUMAP(mlungh, reduction = "pca", dims = 1:30)
# mlungh <- FindNeighbors(mlungh, reduction = "pca", dims = 1:30)
# mlungh <- FindClusters(mlungh, resolution = 0.3)
# 
# mlungh <- PrepSCTFindMarkers(mlungh)
# mlung.combined.markers <- FindAllMarkers(mlungh, # this takes a while
#                                          only.pos = TRUE, 
#                                          min.pct = 0.45, 
#                                          logfc.threshold = 0.25) 
# #mlungh <- FindAllMarkers(mlungh, logfc.threshold = 0.5, min.pct = 0.6, only.pos = T)
# DimPlot(mlungh, label = T, split.by = "author")
...
#| echo: FALSE
hlung.markers <- read.csv("//Users/alexwhitehead/Documents/Postdoc/Bl_Eng2_Priming/Vax_PRRs/human_nostrip_rpca/hlung.combined.markers.csv")

hlung.combined <- readRDS("/Users/alexwhitehead/Documents/Postdoc/Bl_Eng2_Priming/Vax_PRRs/human_nostrip_rpca/h_lung_combined.RDS") 

DefaultAssay(hlung.combined) <- "SCT"

hlung.markers %>% filter(cluster == 1) %>%
  head(100)-> test
write.csv(test$gene, "clust1.csv")

hlung.markers %>% filter(cluster == 9) %>%
  head(100)-> test
write.csv(test$gene, "clust9.csv")

# Here are what the clusters are:
# Cluster 0 = CD74 and HLA-DR, Apoe - Alveolar Macs
# Cluster 1 = MT RNAs, TUBB, CALM1 - Okuda Undefined - Cilliated Cells (based on Metascape)
...
# Here are what the NEW clusters are:
# Cluster 0 = CD74 and HLA-DR, Apoe - Alveolar Macs
# Cluster 1 = MT RNAs, TUBB, CALM1 - Okuda Undefined - Cilliated Cells (based on Metascape)
# Cluster 2 = IL7r, CXCR4, NKG7, CD3e - CD4 T and NK cells
# Cluster 3 = Cldn5, HLA-E - Endothelial cells
# Cluster 4 = SCGB1/3, SAA1, Muc5AC - Secretory cells
# Cluster 5 = Keratins 15,17,19, CD9 - Basal Cells 
# Cluster 6 = CAPS, Tppp3 - Alveolar Type 2

# Cluster 7 = SCGB1/3, SAA1, Muc5AC - Secretory cells
# Cluster 8 = IL7r, CXCR4, NKG7, CD3e - CD4 T and NK cells
# Cluster 9 = Sftpa/b - Type 2 alveolar? - specific to stripp
# Cluster 10 = Jun/ATF3, Cldn4, Sox4  - 
# Cluster 11 = Sftpc, Sftpa  - More secretory cells? - specific to stripp
...
#| label: Label clusters in the human combined object
#| echo: FALSE
mlungh.markers <- read.csv("//Users/alexwhitehead/Documents/Postdoc/Bl_Eng2_Priming/Vax_PRRs/mouse_as_human/mlungh.combined.markers.csv")

mlungh.combined <- readRDS("/Users/alexwhitehead/Documents/Postdoc/Bl_Eng2_Priming/Vax_PRRs/mouse_as_human/m_lung_combined_h.RDS") 
...
# Here are what the clusters are:
# Cluster 0 = SCGB1/3, SFTPC - Secretory/Club 
# Cluster 1 = CD36, PECAM - Endothelial
# Cluster 2 = SFTPC/A/B - Type 2 alveolar
# Cluster 3 = Skap1, Dock2, Ribosomal Proteins - Cilliated?
# Cluster 4 = Chil3, CCL23, CD9, Lyz, MERTK, Axl - Macrophages
# Cluster 5 = Gsn, SLIT2, C7, Angptl1 - Stromal?
# Cluster 6 = CD79A, H2 - B Cells
# Cluster 7 = Scgb1a1, Sftpc - Secretory/Club
# Cluster 8 = Plac8, Lyz, S100A4, 
# Cluster 9 = Scgb1/3, Sftpc - Secretory/Club
# Cluster 10 = Ptma - 
# Cluster 11 = S100A9, Il1b, Cxcl2, Ccl23
# Cluster 12 = Col4a3, Vegfa, 
# Cluster 13 = SPag17, 
# Cluster 14 = CD74, H2, 
# Cluster 15 = Ly6C, Cldn5, PECAM1, CD36 - Lymphatic Endothelial Cells?
# Cluster 16 = SCGB1, 
# Cluster 17 = SCGB1/3, SFTPC - 
# Cluster 18 = POSTN, Itga1, 
# Cluster 19 = Muc16, DCN, C3, 
# Cluster 20 = Foxp1, 
# Cluster 21 = Col3a1, Eln, Col1a1, Col1a2 - Fibroblasts
```
...
RPCA without the stripp samples seems to have best integrated the cells across datasets. Stripp is likely an outlier due to different sample prep and enrichment for basal cells. A few exceptions are: cluster 1 is specific to Okuda (the unidentified cells from their manuscript), and and cluster 10 is the nasal goblet cells that are only found in the Teich nasal data. Moving forward, we will use the h_rpca_ns data for the human lung + nose. This is congruent with the (likely nasal goblet cells) murine approach, which also uses rPCA.
